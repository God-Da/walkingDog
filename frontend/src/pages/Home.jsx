...
const Home = () => {
        ...
        if (window.kakao && window.kakao.maps && window.kakao.maps.Map) {
            ...
            if (!existingScript) {
                ...
                script.onerror = () => {
                    ...
                    setError(
                        "카카오맵을 불러올 수 없습니다. API 키를 확인해주세요."
                    );
                };
    ...
    const fetchSuitability = async (lat, lng) => {
        ...
        try {
            ...
            // 주소를 백엔드로 전달
            const url = new URL(
                "http://localhost:8080/api/walking/suitability"
            );
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    errorData.error || "데이터를 가져오는데 실패했습니다."
                );
            }
    ...
    return (
        <div className="absolute inset-0 w-full h-full overflow-hidden">
            ...
            {/* 정보 패널 - 오버레이 */}
            {showInfoPanel && (
                <div className="absolute top-20 right-4 z-30 w-80 max-h-[calc(100vh-8rem)] overflow-y-auto bg-white rounded-lg shadow-2xl">
